...
export const signUpSchema=Yup.object({
    ...
    ,password:Yup.string().min(6,"password must be 6 letter").matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/, 'Password must contain at least one lowercase letter and one digit').required('Please Enter Your Password'),
    confirm_password:Yup.string().oneOf([Yup.ref("password"), null], "Passwords don't match").required('Confirm your password!')
})
